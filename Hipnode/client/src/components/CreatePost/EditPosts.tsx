...
interface IProps {
  ...
  covers: any;
  setCovers?: Dispatch<SetStateAction<null>>;
  editor: string;
...
const EditPosts = ({
  ...
  const [episodeNumber, setEpisodeNumber] = useState(data?.episode_number || 0);
  ...
  const [checkboxes, setCheckboxes] = useState({
    fulltime: data?.is_fulltime || false,
    parttime: data?.is_partime || false,
    internship: data?.is_internship || false,
    remote: data?.is_remote || false,
    contract: data?.is_contract || false,
    free: data?.is_free || false,
  });

  const [checkboxes2, setCheckboxes2] = useState({
    indiebites: data?.is_indie_bites || false,
    softwaresocial: data?.is_software_social || false,
    hipnode: data?.is_hipnode || false,
    free: data?.is_free || false,
  });
  ...
  // console.log('covers', typeof covers, covers);
    ...
    <main className="flex items-center justify-center dark:bg-dark-black-1 min-h-screen py-8 px-8 md:px-2 relative">
      <section className="w-full max-w-[1200px] flex items-center flex-col gap-4">
        <div className="w-full max-w-[880px] bg-white dark:bg-dark-black-3 p-6 rounded-[16px]">
          ...
          {category == 'Posts' && <Tags tags={tags} setTags={setTags} />}
